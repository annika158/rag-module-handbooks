...
###### RAG-SYSTEM FOR MODULE HANDBOOKS - with chunk modifications (metadata included) and without metadata filtering ######
...
def main():
    ...
    # Load vectorstore with the modified chunks
    persist_directory = "vectorstores"
    collection_name = "module_handbooks_economics_modified_chunks" ########
    vectorstore = Chroma(
    ...
    for example in examples:
        ...
        result_file = f"result_comparison/metadata_in_chunk_retrieved_chunks/question{i}.txt" ########

        def format_splits(relevant_splits):
            """
            Join the relevant splits from the retriever and save their information to a text file.
            """
            all_metadata_str = ""
            all_splits_str = ""
            ...
            with open(result_file, "w") as f:
                f.write(f"Metadata in chunk - Question: {question} \n\n") ########
                f.write("===== Metadata =====\n\n")
