...
###### RAG-SYSTEM FOR MODULE HANDBOOKS - with manual metadata filtering ######
...
examples = [
    ...
    },
    {
        ...
        "filter": {
            "$and": [
                {"source": {"$eq": "module_handbooks_economics/2022-09-23_msc_is_module_compendium.pdf"}},
                {"module_title": {"$eq": "Master’s Thesis  "}},
            ]
        },
    },
    {
        ...
        "filter": {
            "$and": [
                {"source": {"$eq": "module_handbooks_economics/2020-04-27_msc_ec_modulbeschreibungen.pdf"}},
                {"module_title": {"$eq": "Regulierungsökonomik "}},
            ]
        },
    },
    {
        ...
        "filter": {
            "$and": [
                {"source": {"$eq": "module_handbooks_economics/2020-06-16_bsc_vwl_modulbeschreibungen.pdf"}},
                {"module_title": {"$eq": "Grundlagen der Regulierung "}},
            ]
        },
    },
    {
        ...
        "filter": {
            "$and": [
                {"source": {"$eq": "module_handbooks_economics/2022-12-08_bsc_bwl_modulhandbuch.pdf"}},
                {"module_title": {"$eq": "Bachelorarbeit  "}},
            ]
        },
    },
    {
        ...
        "filter": {
            "$and": [
                {"source": {"$eq": "module_handbooks_economics/2024-03-05_msc_bwl_modulhandbuch.pdf"}},
                {"module_title": {"$eq": "Performance Management & Strategy Execution "}},
            ]
        },
    },
    {
        ...
        "filter": {
            "$and": [
                {"source": {"$eq": "module_handbooks_economics/2024-03-05_msc_bwl_modulhandbuch.pdf"}},
                {"module_title": {"$eq": "Praxisworkshop "}},
            ]
        },
    },
]
...
result_file = f"result_comparison/metadata_filter_source_module_retrieved_chunks/question{i+1}.txt"
...
def format_splits(relevant_splits):
    ...
    with open(result_file, "w") as f:
        f.write(f"Metadata filter - Question: {question} \n\n")  ########
        f.write("===== Metadata =====\n\n")
